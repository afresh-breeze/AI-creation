...
- **核心玩法**：小球在持续向前的三维赛道上**左右移动**并**跳跃**，躲避障碍，收集积分；达到当前关目标分数后出现过关界面，可进入下一关。
- **关卡结构**：共 **9 个有目标分数的关卡** + **第 10 关无限模式**（无目标分数，可一直挑战）。
- **胜负条件**：
  - 碰到障碍物、或累计前进距离达到配置上限（`maxDistance`）→ 触发结束流程（在拥有复活次数时可先进入复活确认）。
  - 收集**红色积分**时，除加分外还会按该球标注的**死亡概率**掷骰，失败则视同死亡（可走复活流程）。
...
若你正在对比学习或二次开发，建议同时打开 **`index_beautify.html`** 中的 `config`、`levels`、`checkCollisions`、`checkLevelUp` 与 `saveGameState` / `loadGameState` 四处对照阅读，即可快速建立对规则与数据流的完整心智模型。
